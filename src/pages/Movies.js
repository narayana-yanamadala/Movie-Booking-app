...
export default function Movies() {
  ...
  let filtered = movies.filter(m => {
    if (tab === 'upcoming') return m.upcoming;
    if (tab === 'trending') return m.trending;
    return !m.upcoming;
  });
  if (search) filtered = filtered.filter(m => m.title.toLowerCase().includes(search.toLowerCase()) || m.genre.some(g => g.toLowerCase().includes(search.toLowerCase())));
  if (genre !== 'All') filtered = filtered.filter(m => m.genre.includes(genre));
      ...
      <div className="section">
        ...
        <div style={{ marginBottom: '2rem' }}>
          <h1 className="section-title" style={{ marginBottom: '1rem' }}>
            {search ? `Results for "${search}"` : '🎬 All <span>Movies</span>'}
          </h1>
          {search && <p className="text-muted">{filtered.length} movies found</p>}
        </div>
